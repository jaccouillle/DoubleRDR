...
$[$Prof$]$ $<$Jean-Guillaume Dumas$>$\\
$[$Prof$]$ $<$Jean-Louis Roch$>$\\
$[$Prof$]$ $<$Vanessa Vitse>\\
}
...
\begin{center}
    \begin{tikzpicture}
      \draw [black, thick, ->] (1,4) -- (2,4);
      \node [above] at (1.5,4) {$k_1,k_2$};
      \draw [thick, fill=cyan] (2,2) rectangle (10,6);
      \node at (6,4) {$DoubleRDR$};
      \node at (2.3,2.3) {$A_0$};
      \draw [black, thick, ->] (6,7) -- (6,6);
      \node at (5.5,6.5) {$\mathcal{D} = \{1,\ldots\}$};
      \draw [black, thick, ->] (10,4) -- (16,4);
      \node [above] at (13,4) {$DoubleRDR_{k_1,k_2} =$};
      \node [below] at (13,4) {$\begin{pmatrix} d_0 & 0 & \ldots & 0 & d_1 & \ldots & d_n \\ 0 & d_0' & \ldots & 0 & d_1' & \ldots & d_m' \end{pmatrix}$};
   \end{tikzpicture}
\end{center}

Les entrées du programme sont :
\begin{itemize}
 \item [$\bullet$] les exposants $k_1$ et $k_2$,
 \item [$\bullet$] l'ensemble $\mathcal{D}$ des coefficients possibles.
\end{itemize}
...
\begin{center}
    \begin{tikzpicture}
      \draw [black, thick, ->] (1,6) -- (2,6);
      \node [above] at (1.5,6) {$k_1,k_2$};
      \draw [thick, fill=white] (2,2) rectangle (15,9);
      \node at (2.3,2.3) {$A_0$};
      \node [above] at (4,9) {$DoubleRDR$};
      \draw [black, thick, -] (2,6) -- (5,6);
      \draw [black, thick, ->] (5,6) -- (8,7);
      \node at (6.5,7.5) {$k_1 < 2^{W_{max}}$ ou};
      \node at (6.5,7) {$k_2 < 2^{W_{max}}$};
      \draw [black, thick, ->] (5,6) -- (8,5);
      \node at (6.5,5) {$k_1 \geq 2^{W_{max}}$ et};
      \node at (6.5,4.5) {$k_2 \geq 2^{W_{max}}$};
      \draw [thick, fill=cyan] (8,4) rectangle (12,5.5);
      \node at (8.3,4.3) {$A_{01}$};
      \node at (10,4.75) {remplir $DoubleRDR$};
      \draw [black, thick, -] (12,4.75) -- (14,4.75);
      \node [above] at (13,4.75) {$k_1',k_2'$};
      \draw [black, thick, -] (14,4.75) -- (14,2.75);
      \draw [black, thick, -] (14,2.75) -- (3.5,2.75);
      \draw [black, thick, ->] (3.5,2.75) -- (3.5,6);
      \node at (3.5,5.5) {$k_1 = k_1'$};
      \node at (3.5,5) {$k_2 = k_2'$};
      \draw [thick, fill=cyan] (8,6.5) rectangle (12,8);
      \node at (8.3,6.8) {$A_{02}$};
      \node at (10,7.25) {fin $DoubleRDR$};
      \draw [black, thick, -] (12,7.25) -- (14,7.25);
      \draw [black, thick, -] (14,7.25) -- (15,6);
      \node [above] at (13.5,7.25) {$DoubleRDR_{k_1,k_2}$};
      \draw [black, thick, ->] (9.5,10) -- (9.5,9);
      \node at (9.5,9.5) {$\mathcal{D} = \{1,\ldots\}$};
      \draw [black, thick, ->] (9.5,9) -- (9.5,8);
      \node at (9.5,3.5) {$\mathcal{D} = \{1,\ldots\}$};
      \draw [black, thick, ->] (9.5,3) -- (9.5,4);
      \draw [black, thick, ->] (15,6) -- (17,6);
      \node [above] at (16,6) {$DoubleRDR_{k_1,k_2}$};
   \end{tikzpicture}
\end{center}

On remarque qu'il faut fonctionner différemment pour la fin du recodage (ie $k_i < 2^{W_{max}}$), c'est pourquoi
le programme applique la méthode $DoubleRDR$ en recalculant $k_1$ et $k_2$ au fur et à mesure jusqu'à ce 
que l'un des deux exposants soit petit, on applique alors l'algorithme spécialement conçu pour ce cas particulier.

\begin{center}
    \begin{tikzpicture}
      \draw [black, thick, ->] (1,4) -- (2,4);
      \node [above] at (1.5,4) {$k_1,k_2$};
      \draw [thick, fill=white] (2,2) rectangle (12,6);
      \node at (2.3,2.3) {$A_{01}$};
      \node [above] at (4,6) {remplir $DoubleRDR$};
      \draw [black, thick, ->] (2,4) -- (3,4);
      \draw [thick, fill=cyan] (3,3) rectangle (5,5);
      \node at (4,4.3) {calcul bit};
      \node at (4,3.7) {faible}; 
      \draw [black, thick, ->] (5,4) -- (7,4);
      \node [above] at (6,4) {$B_{f,1},B_{f,2}$};
      \node [below] at (6,4) {$k_1',k_2'$};
      \draw [thick, fill=cyan] (7,3) rectangle (10,5);
      \node at (7.4,3.3) {$A_{011}$};
      \node at (8.5,4.3) {choix};
      \node at (8.5,3.7) {coefficient(s)};
      \draw [black, thick, -] (10,4) -- (12,4);
      \node [above] at (11,4) {$k_1'',k_2''$};
      \draw [black, thick, ->] (7.5,6) -- (8.5,5);
      \draw [black, thick, ->] (7.5,7) -- (7.5,6);
      \node at (7.5,6.5) {$\mathcal{D} = \{1,\ldots\}$};
      \draw [black, thick, ->] (12,4) -- (14,4);
      \node [above] at (13,4) {$k_1 = k_1''$};
      \node [below] at (13,4) {$k_2 = k_2''$};
   \end{tikzpicture}
\end{center}

De la même manière que dans l'exemple, il est très important de calculer les bits faibles de chaque exposant
avant de calculer le coefficient associé sous peine de choisir le mauvais coefficient.

\begin{center}
    \begin{tikzpicture}
      \draw [black, thick, ->] (0,4) -- (2,4);
      \node [below] at (1,4) {$k_1,k_2$};
      \node [above] at (1,4) {$B_{f,1},B_{f,2}$};
      \draw [thick, fill=white] (2,2) rectangle (12,6);
      \node at (2.4,2.3) {$A_{011}$};
      \node [above] at (4,6) {choix coefficient(s)};
      \draw [black, thick, -] (2,4) -- (3,4);
      \draw [black, thick, ->] (3,4) -- (5,5);
      \node at (4,5) {$B_{f,1} < B_{f,2}$};
      \draw [thick, fill=cyan] (5,2.6) rectangle (9,3.4);
      \node at (7,3) {Choix coeff $k_2$ };
      \draw [black, thick, -] (9,5) -- (11,4);
      \draw [black, thick, ->] (3,4) -- (5,4);
      \node at (4,4.2) {$B_{f,1} = B_{f,2}$};
      \draw [thick, fill=cyan] (5,3.6) rectangle (9,4.4);
      \node at (7.2,4) {Choix coeff $(k_1,k_2)$ };
      \draw [black, thick, -] (9,4) -- (11,4);
      \draw [black, thick, ->] (3,4) -- (5,3);
      \node at (4,3) {$B_{f,1} > B_{f,2}$};
      \draw [thick, fill=cyan] (5,4.6) rectangle (9,5.4);
      \node at (7,5) {Choix coeff $k_1$ };
      \draw [black, thick, -] (9,3) -- (11,4);
      \node [above] at (11.5,4) {$k_1', k_2'$};
      \draw [black, thick, -] (11,4) -- (12,4);
      \draw [black, thick, ->] (7.5,7) -- (7.5,6);
      \draw [black, thick, ->] (7.5,6) -- (7,5.4);
      \node at (7.5,6.5) {$\mathcal{D} = \{1,\ldots\}$};
      \draw [black, thick, ->] (12,4) -- (14,4);
      \node [above] at (13,4) {$k_1 = k_1'$};
      \node [below] at (13,4) {$k_2 = k_2'$};
...
\begin{algorithm}
 \caption{Algorithme de calcul de $d_0$ et $d_0'$ lorsque $B_{f,1} = B_{f,2}$}
 \begin{algorithmic}
  \REQUIRE les exposants $k_1$ et $k_2$ et l'ensemble $\dbarre$ avec $W_n$ (borne).
  \ENSURE $[d_0,d_0']$ avec $d_0, d_0' \in \dbarre$.
  \FOR{$i$ allant de $0$ à $|\dbarre| - 1$}
  \STATE $KmoinsDbarre_1[i] \leftarrow k_1 - \dbarre[i]$ // l'ensemble $k_1 - \dbarre$
  \ENDFOR
  \FOR{$i$ allant de $0$ à $|\dbarre| - 1$}
  \STATE $KmoinsDbarre_2[i] \leftarrow k_2 - \dbarre[i]$ // l'ensemble $k_2 - \dbarre$
  \ENDFOR
  \STATE $pow \leftarrow 2^{Wn+2}$ // la variable $pow$ correspond à $w_{max}$
  \STATE $c \leftarrow 0$ // valeur d'arrêt qui arrête le programme quand on trouve les coefficients
  \WHILE{$pow > 1$ et $c \neq 1$}
  \STATE $c1 \leftarrow 0$ // valeur d'arrêt pour $d_0$
  \STATE $c2 \leftarrow 0$ // valeur d'arrêt pour $d_0'$
  \STATE $i \leftarrow 0$
  \WHILE{$i < |\dbarre|$ et $c_1 \neq 1$}
  \STATE $S \leftarrow KmoinsDbarre_1[i]$
  \IF{$S = 0 \mod (pow)$ et $S \neq 0 \mod (2 \times pow)$}
  ...
  \STATE $S \leftarrow KmoinsDbarre_2[j]$
  \IF{$S = 0 \mod (pow)$ et $S \neq 0 \mod (2 \times pow)$}
  ...
  \STATE $c \leftarrow c1 \times c2$ // $c$ vaut $1$ uniquement lorsque $c_1$ et $c_2$ valent $1$
  \ENDWHILE
...
\end{algorithm}    
      ...
      \subsubsection{Remplissage de $DoubleDmax$ : Première fonction}

      L'algorithme $11$ correspondant aux deux premiers procédés de la partie $5.2.3$ (avec $0$ ajouté au tableau $\dbarre$),
...
\begin{algorithm}
 ...
 \begin{algorithmic}
  ...
  \STATE $pow_1 \leftarrow pow$
  \WHILE{$pow_1 \leq 2^{Wn+2}$}
...
\end{algorithm}
     ...
     \subsubsection{Remplissage de $DoubleDmax$ : Deuxième fonction}
     
    L'algorithme $12$ permet de traiter le troisième procédé de la partie $5.2.3$,
 ...
 \begin{algorithmic}
  ...
  \WHILE{$pow_1 \leq 2^{W_n+2}$}
  \STATE $d_1 \leftarrow pow_1$

  \STATE $pow_2 \leftarrow pow$
  \WHILE{$pow_2 \leq 2^{W_n+2}$}
...
\begin{thebibliography}{1}
 ...
 \bibitem{a}
 N.Méloni et A.Hasan
 \newblock {\em Random Digit Representation of Integers}.
\newblock 2015.

\bibitem{b}
B.Möller
\newblock {\em Algorithms for multi-exponentiation}.
\newblock Darmstadt University, 2001.

\bibitem{c}
J.Solinas
\newblock {\em Low-Weight Binary Representations for Pairs of Integers}.
